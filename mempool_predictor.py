...
def preprocess_data(data, is_fees_data=False):
    P_t = 58643  # Updated: Use current Bitcoin price based on analysis
    if is_fees_data:
...
def main():
    ...
    if mempool_data:
        ...
        for period, day_count in zip(time_periods, days):
            ...
            print(f"{period}:")
            print(f"  Projected Transaction Count: {projected_tx_count}")
            print(f"  Projected Total Fees: {int(projected_total_fees)} satoshis")
    else:
